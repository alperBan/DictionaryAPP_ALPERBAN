...
class SearchViewController: UIViewController, LoadingShowable {
    ...
    var searchList = [RecentSearch]()
       ...
       @IBAction func searchButton(_ sender: Any) {
           ...
           if word.isEmpty {
               let alert = UIAlertController(title: "Warning", message: "Enter a Word!", preferredStyle: .alert)
               let cancelButton = UIAlertAction(title: "Cancel", style: .cancel, handler: nil)
               alert.addAction(cancelButton)
               self.present(alert, animated: true, completion: nil)
           } else {
               // Check if the word already exists in the search list
               if let existingIndex = searchList.firstIndex(where: { $0.searchList == word }) {
                   // Word already exists, remove it from its current position and insert at the beginning
                   let existingSearch = searchList.remove(at: existingIndex)
                   searchList.insert(existingSearch, at: 0)
               } else {
                   // Word doesn't exist, create a new recent search object and insert at the beginning
                   let recentSearch = RecentSearch(context: context)
                   recentSearch.searchList = word
                   searchList.insert(recentSearch, at: 0)
                   
                   // Remove the last search if the list exceeds a certain limit
